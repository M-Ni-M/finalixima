...
export const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    auctionSocket.on("auctionStarted", (auctionData) => {
      ...
      // If this is the auction we're viewing, update the status
  ...
  const handleStartAuction = async () => {
    try {
      ...
      // Call your API to start the auction (you'll need to implement this)
      // For example: await apiStartAuction(id);
  ...
  const handleJoinAuction = () => {
    // Navigate to the auction room when joining
    navigate(`/room/${id}`);
  };
      ...
      <section className="w-full min-h-screen flex items-center bg-[#F2F2F2] pt-[4vh] gap-5 px-5">
        ...
        <div className="flex flex-col w-[30%] min-h-[70vh] rounded-2xl border bg-white border-gray-300 shadow-2xl mt-16">
          ...
          <ul className="p-5 space-y-1 font-[MuseoModerno] text-l flex flex-col gap-5">
            <li>
              <strong>Item ID:</strong> {item._id || item.id}
            </li>
            <li>
        ...
        <div className="flex flex-col w-[30%] min-h-[70vh] rounded-2xl bg-white font-[MuseoModerno] border border-gray-300 shadow-2xl mt-16">
          ...
          <div className="space-y-0.5 p-5 text-l flex flex-col gap-5">
            ...
            <div className="mt-6 space-y-4">
              {/* Always show Join Auction button for non-owners */}
              {!isOwner && (
                <button
                  onClick={handleJoinAuction}
                  className="w-full py-3 font-bold bg-green-500 text-white rounded-md hover:bg-green-600"
                >
                  Join Auction
                </button>
              )}
              
              {/* Show status notification based on auction state */}
              {item.status === "active" && (
              ...
              {isOwner && item.status === "upcoming" && (
                <div className="bg-blue-100 border border-blue-400 p-4 rounded-md">
                  <p className="text-blue-800 font-bold">You are the owner of this item</p>
